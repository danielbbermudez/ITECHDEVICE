...
function Item({ item, stock, name, price, description, picture }) {
  ...
  return (
    <Card key={item.id} className="bg-light h-100 shadow-lg p-3 mb-3 mr-2 ml-2 rounded">
      ...
      <span className="position-absolute top-4 end-4 translate-middle badge rounded-pill bg-primary">{isInCart(item) ?
      ...
      <Container className="d-flex justify-content-center text-center align-middle w-100 mw-30">
        <Badge className="bg-dark mb-4 text-center m-1">Stock {stock}</Badge>
        <Badge bg="dark me-2 mb-4 text-center m-1">$ {price} COP</Badge>
      </Container>
      <LinkContainer to={`/item/${item}`}><Button className="btn btn-primary">Ver detalles</Button></LinkContainer>
    </Card>
